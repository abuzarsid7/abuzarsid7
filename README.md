🚀 Abuzar Siddiqui
================================
Software Developer | C++ DSA | Full Stack | iOS
------------------------------------------------
📍 Gautam Buddha Nagar, India

📧 [Email](mailto:abuzarsid7456@gmail.com) | 🔗 [LinkedIn](https://www.linkedin.com/in/abuzarsid)￼ | [GitHub](https://github.com/abuzarsid7/abuzarsid7)
-----------------------------------------------------------------------------------------------------------
## 👨‍💻 About Me

I’m a Computer Science undergraduate passionate about building scalable software systems and solving problems using Data Structures & Algorithms in C++.
I have hands-on experience in:

* Full-stack web development
* Backend systems & APIs
* iOS app development
* Deployment, version control, and databases

### ⚡ Tech Stack

💻 Languages

* C++ (DSA)
* Java
* Swift
* Python

🧠 Core Skills

* Data Structures & Algorithms
* Object-Oriented Programming
* System Design (Basics)
* iOS App Development

🌐 Development

* Frontend & Backend Development
* REST APIs
* Authentication Systems

🛠 Tools & Technologies

* Git & GitHub
* Supabase / Firebase
* Xcode
* CI/CD Basics
* Agile/Scrum

  -----------------------------------------------------------------
 ### 📈 What I’m Focused On

* Building production-ready full stack projects
* Strengthening core CS fundamentals (OS, DBMS, CN)
* Writing clean, scalable, maintainable code

  --------------------------------------------------------------
###  🤝 Let’s Connect

If you’re working on something interesting or want to partner up for something, feel free to connect!
